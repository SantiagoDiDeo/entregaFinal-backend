...
productRouter.get("/", async (req, res) => {
  try {
    ...
    logger.error(
      `${err} --Error al intentar renderizar lista de productos route`
    );
  }
});
...
productRouter.post("/", async (req, res) => {
  try {
    ...
    const newCart = await createCartController(
      newData.username,
      newData.address
    );
    if (newCart) {
...
productRouter.get("/:id", async (req, res) => {
  try {
    ...
    logger.error(
      `${err} --Error al intentar renderizar productos por id route`
    );
  }
});
...
productRouter.get("/category/:category", async (req, res) => {
  try {
    ...
    logger.error(
      `${error} --Error al intentar renderizar productos por categoria`
    );
  }
});
