...
import { deleteCartController, deleteCartItemController, getCartByUsernameController, updateCartController } from '../controller/carts.js';
import {getOrderByUsernameController, newOrderController} from '../controller/orders.js';
...
cartRouter.get('/' , async (req, res) => { //falta el authenticate
...
cartRouter.post('/order', async (req, res) => {
    try {
        ...
        res.send(`orden de compra realizada, ${JSON.stringify( realizedOrder,null,2)}` );
    } catch (error) {
    logger.error(`${error} --Error al intentar realizar "post" de orden ${order}`);
    };
});

cartRouter.delete('/:id/:title', async (req, res) => {
    try {
        const deleted = await deleteCartItemController( req.params.id, req.params.title);
