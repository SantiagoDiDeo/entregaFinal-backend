...
const getCartByUsernameController = async (username) => {
  try {
    ...
    logger.error(
      `${error} --Error al intentar encontrar usuario controller: ${username} `
    );
  }
};
...
const updateCartController = async (cartId, updatedCartData) => {
  try {
    ...
    logger.error(
      `${error} --Error al intentar actualizar carrito controller id: ${cartId}`
    );
  }
};
...
const deleteCartController = async (cartId) => {
  try {
    ...
    logger.error(
      `${error} --Error al intentar eliminar carrito controller id: ${cartId}`
    );
  }
};
...
const deleteCartItemController = async (cartId, itemTitle) => {
  try {
    ...
    logger.error(
      `${error} --Error al intentar borrar item de carrito controller id: ${cartId}`
    );
  }
};
